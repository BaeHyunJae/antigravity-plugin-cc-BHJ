...
argument-hint: "[--background|--wait] [--sandbox|--read-only] [--continue] [--conversation <id>] [--add-dir <path>] [--model <slug>] [--effort <level>] [--no-slash-commands] [--agy-arg <token>] [what Antigravity should build, investigate, or fix]"
...
2. **Read the execution flags, then strip them from the task text.** `--background` and `--wait` control how *you* run the subagent — they are not part of the natural-language task and must NOT be forwarded as task text:
   ...
   Everything else — the task description plus companion flags `--sandbox`, `--read-only`, `--continue`, `--conversation <id>`, `--add-dir <path>`, `--model <slug>`, `--effort <level>`, `--no-slash-commands`, `--agy-arg <token>` — is forwarded to the subagent verbatim as its prompt.
